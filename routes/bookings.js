...
router.post(
    ...
    asyncHandler(async (req, res, next) => { // Wrapped with asyncHandler
        // ... (Keep the existing POST /api/bookings logic exactly as it was) ...
        ...
        const newBookingData = {
            ...
            totalFare: parseFloat(fare), // Assuming schema uses 'totalFare'
...
router.get(
    ...
    asyncHandler(async (req, res, next) => { // Wrapped with asyncHandler
        ...
        // 1. Fetch user's bookings with populated details
        const bookings = await Booking.find({ user: userId })
            .populate({
                path: 'schedule',
                select: 'routeOrigin routeDestination departureDateTime arrivalDateTime vehicle driver', // Select needed fields
                populate: [ // Nested population
                    { path: 'vehicle', select: 'model registrationNumber capacity type' }, // Populate vehicle details
                    { path: 'driver', select: 'name phone' } // Populate driver details
                ]
            })
        ...
        if (!Array.isArray(bookings)) {
             console.error("[GET /my-bookings] Query returned non-array:", bookings);
             return next(new ErrorResponse('Error fetching bookings data.', 500));
        }

        // 2. Get IDs of completed bookings for this user
        const completedBookingIds = bookings
            .filter(b => b.status === 'Completed')
            .map(b => b._id); // Keep as ObjectIds for the query

        // *** ADDED LOGGING ***
        console.log(`[GET /my-bookings] Completed Booking IDs found:`, completedBookingIds.map(id => id.toString())); // Log as strings for readability

        // 3. Find which of these completed bookings have reviews by this user
        let reviewedBookingIdSet = new Set();
        let foundReviews = []; // *** ADDED LOGGING ***
        if (completedBookingIds.length > 0) {
            // Find reviews where the booking ID is in our list AND the user matches
            const reviews = await Review.find({
                booking: { $in: completedBookingIds }, // Use ObjectId array here
                user: userId
            }).select('booking'); // Select only the booking field for efficiency

            foundReviews = reviews; // *** ADDED LOGGING ***

            // Add the string representation of the booking ObjectId to the Set
            ...
            console.log(`[GET /my-bookings] Set of reviewed booking IDs:`, reviewedBookingIdSet);
             ...
             console.log(`[GET /my-bookings] No completed bookings found for user ${userId}.`);
        }

        // 4. Add 'isReviewed' flag to each booking object
        const bookingsWithReviewStatus = bookings.map(booking => {
            const bookingIdString = booking._id.toString(); // Get string ID for lookup
            ...
            const isReviewedFlag = isCompleted && hasReview;
             ...
             // *** ADDED LOGGING ***
             if (isCompleted) {
                 console.log(`[GET /my-bookings] Checking booking ${bookingIdString}: Completed=${isCompleted}, HasReview=${hasReview}, Setting isReviewed=${isReviewedFlag}`);
             }

            return {
                ...booking,
                // Add the flag: true if booking is Completed AND its ID is in the reviewed set
                isReviewed: isReviewedFlag
            };
        });
        ...
        res.status(200).json({
            ...
            data: bookingsWithReviewStatus, // Send bookings with the added flag
        });
    })
);
...
router.put(
    ...
    asyncHandler(async (req, res, next) => { // Wrapped with asyncHandler
        ...
        const updatedBooking = await Booking.findById(bookingId)
             .populate('user', 'name')
             .populate({ path: 'schedule', populate: [{ path: 'vehicle' }, { path: 'driver' }]})
             .lean();
